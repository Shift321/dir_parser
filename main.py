...
from utils.files import delete_files, get_all_hashes_from_db
...
def main(data):
    ...
    difference = list(set(data['file_hashes']).symmetric_difference(set(get_all_hashes_from_db(session=session))))
    if len(difference) != 0:
